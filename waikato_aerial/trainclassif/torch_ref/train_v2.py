...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None) -> float:
    model.train()
    ...
    if wandb.run:
        wandb.log({
            ...
            "train/acc1": metric_logger.acc1.global_avg,
            "train/acc5": metric_logger.acc5.global_avg,
...
def evaluate(model, criterion, data_loader, device, print_freq=100, log_suffix="") -> float:
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
...
class HFDatasetAdapter(Dataset):
    """Adapter for HuggingFace datasets to work with PyTorch training pipeline"""
    ...
    def __getitem__(self, idx) -> tuple[Any, Any]:
        ...
        if 'image' in item:
            ...
            raise ValueError(f"Can't find image data in dataset item keys: {item.keys()}")
        ...
        if 'label' in item:
            ...
            raise ValueError(f"Can't find label in dataset item keys: {item.keys()}")
...
def load_hf_data(args) -> tuple[Dataset, Dataset, torch.utils.data.Sampler, torch.utils.data.Sampler]:
    ...
    hf_dataset = datasets.load_dataset(args.data_path)
    
    # Get train and validation splits
    if 'train' in hf_dataset:
        train_dataset_raw = hf_dataset['train']
    else:
        raise ValueError("No training split found in the dataset.")
    
    if 'validation' in hf_dataset:
        val_dataset_raw = hf_dataset['validation']
    elif 'val' in hf_dataset:
        val_dataset_raw = hf_dataset['val']
    elif 'test' in hf_dataset:
        val_dataset_raw = hf_dataset['test']
    else:
    ...
    if args.weights and args.test_only:
        ...
        if args.backend == "tensor":
            val_transforms = torchvision.transforms.Compose([torchvision.transforms.PILToTensor(), val_transforms])
    ...
    dataset = HFDatasetAdapter(train_dataset_raw, transform=train_transforms)
    dataset_test = HFDatasetAdapter(val_dataset_raw, transform=val_transforms)
...
def load_data(traindir, valdir, args) -> tuple[Dataset, Dataset, torch.utils.data.Sampler, torch.utils.data.Sampler]:
    # Data loading code
    print("Loading data")
    val_resize_size, val_crop_size, train_crop_size = (
        args.val_resize_size,
...
def main(args) -> None:
    ...
    if args.use_hf_dataset:
        dataset, dataset_test, train_sampler, test_sampler = load_hf_data(args)
    else:
    ...
    wandb.init(project=args.wandb_project, name=args.wandb_run, config=wandb_config)
    ...
    best_epoch = -1
    best_val_acc = 0.0
    train_acc_at_best_epoch = 0.0
    epochs_no_improve = 0

    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed:
            train_sampler.set_epoch(epoch)
        epoch_train_acc = train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema, scaler)
        lr_scheduler.step()
        epoch_val_acc = evaluate(model, criterion, data_loader_test, device=device)
        if model_ema:
            epoch_val_acc = evaluate(model_ema, criterion, data_loader_test, device=device, log_suffix="EMA")
        
        if epoch_val_acc > best_val_acc:
            best_epoch = epoch
            best_val_acc = epoch_val_acc
            train_acc_at_best_epoch = epoch_train_acc
            epochs_no_improve = 0
            ...
            if epochs_no_improve >= args.early_stopping_patience:
                ...
                print(f"Best epoch: {best_epoch}, Best val acc: {best_val_acc:.3f}, Train acc at best epoch: {train_acc_at_best_epoch:.3f}")
                break
    ...
    if wandb.run:
        wandb.summary({
            ...
            "train_acc_at_best_epoch": train_acc_at_best_epoch,
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--wandb-project", required=True, type=str, help="WandB project name")
    parser.add_argument("--wandb-run", required=True, type=str, help="WandB run name")

    # HF dataset parameters
    parser.add_argument("--use-hf-dataset", action="store_true", help="Use HuggingFace dataset instead of local files")

    # Early stopping parameters
    parser.add_argument("--early-stopping-patience", default=10, type=int, help="Number of epochs with no improvement after which training will be stopped")
    return parser
