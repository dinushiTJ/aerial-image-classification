# Generating Textual Inversion Images
...
def _get_classes(version: str, images_per_prompt: int) -> dict[str, str]:
    ...
    for cls, token in classes.items():
        class_dir = f"{img_dest}{version}/{cls}"
...
def _generate(
    ...
    ti_model_id = f"dushj98/{class_token.lower()}_textual_inversion"
    pipe.load_textual_inversion(ti_model_id)
...
def _push_to_hub(huggingface_token: str, version: str) -> None:
    dataset = load_dataset("imagefolder", name=f"waikato_aerial_2017_synthetic_ti{version}", data_dir=f"{img_dest}{version}")
    dataset.push_to_hub(f"dushj98/waikato_aerial_2017_synthetic_ti{version}", token=huggingface_token)
    
    click.echo(f"✅ The generated dataset was uploaded to huggingface\n")
...
def generate(version_tag: str, images_per_prompt: int, num_inference_steps: int, huggingface_token: str, use_cpu: bool, no_push: bool):
    ...
    for class_name, class_token in classes.items():
    ...
    if not no_push:
        _push_to_hub(huggingface_token=huggingface_token, version=version_tag)
