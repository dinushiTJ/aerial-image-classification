...
def load_data(
    ...
    def transform_dataset(example):
        if hasattr(example["img"], "convert"):
...
def load_data_with_augmentation(
    ...
    def transform_dataset(example, split):
        ...
        if split == "train":
            example["img"] = train_transform(example["img"])
        else:
            example["img"] = val_transform(example["img"])
        return example
    ...
    for epoch in epoch_pbar:
        ...
        for phase in ["train", "val"]:
            ...
            for batch in batch_pbar:
                inputs, labels = batch["img"].to(dev), batch["label"].to(dev)
