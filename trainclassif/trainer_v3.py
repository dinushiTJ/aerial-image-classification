...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Use feature extraction (True) vs fine-tuning (False)",
)
@click.option(
    "--metadata",
    "-m",
...
@click.option(
    ...
    type=str,
    default="",
    help="HuggingFace repository ID for uploading (e.g., username/model-name)",
)
def train(
    dataset_name: str,
    hf_token: str,
    ...
    # Add feature extraction mode to metadata
    ...
    for model_name in models_to_train:
        ...
        if use_wandb:
            ...
            wandb_run_name = (
                f"{model_name}_{run_name}_{'tl' if feature_extract else 'ft'}"
            )

            try:
                wandb.init(
                    project=f"model-training-{run_name}",
                    name=wandb_run_name,
                    config=config,
                )
        ...
        trained_model, results, checkpoint_path = train_model(
            ...
            num_epochs=epochs,
            wandb_log=use_wandb,
            model_name=f"{model_name}_{run_name}_{'tl' if feature_extract else 'ft'}",
        )
        ...
        if upload_to_hf:
            ...
            if not model_repo_id:
                ...
                model_repo_id = f"dushj98/aerial-{model_name.replace('_', '-')}-{run_name.replace('_', '-')}-{'tl' if feature_extract else 'ft'}"
